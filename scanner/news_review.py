...
def review_latest(store,ticker,document_ids,now):
    ...
    for identity in document_ids[:3]:
        ...
        try:
            ...
            instructions+=' Publication time is not event time. If an excerpt repeats a previously announced event (including an explicitly older event date), describes a past price rally, or lacks evidence of a new development, use NEUTRAL. Relative dates refer to the supplied publication time. A newly published retrospective is not a fresh catalyst.'
            response=client.responses.create(model=model,instructions=instructions,input='Return the classification as JSON.\n'+json.dumps({'ticker':ticker,'published_at':doc['published_at'],'reviewed_at':now.isoformat(),'title':doc['title'],'body':doc['text'][:16000]}),text={'format':{'type':'json_object'}})
